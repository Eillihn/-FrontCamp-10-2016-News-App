...
/***/ function(module, exports, __webpack_require__) {
	...
	var NewsFeed = __webpack_require__(300);
	...
	var NewsFeed = function () {
	    ...
	    _createClass(NewsFeed, [{
	        ...
	        value: function attachEvents() {
	            ...
	            this.sourceEl.addEventListener('change', function (e) {
	                _this.source = e.target.value;
	                _this.update();
	            });
	        }
	    }, {
	        ...
	        value: function update() {
	            ...
	            fetch(ARTICLES_URL + '?source=' + source + '&apiKey=' + API_KEY).then(function (response) {
	                return response.json();
	            }).then(this.display.bind(this)).catch(function (error) {
	                console.log(error);
	            });
	        }
	    }, {
	        ...
	        key: 'createArticlesHtml',
	        value: function createArticlesHtml() {
	            var articles = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : [];

	            var html = [];

	            html = articles.map(function (article) {
	                var date = NewsFeed.getFormattedDate(article.publishedAt);
	        ...
	        value: function init() {
	            ...
	            fetch(SOURCES_URL).then(function (response) {
	                return response.json();
	            }).then(function (data) {
	                _this2.createSources(data);
	                _this2.update(_this2.options.INITIAL_SOURCE);
	        ...
	        key: 'createSourcesHtml',
	        value: function createSourcesHtml() {
	            var _this3 = this;

	            var sources = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : [];

	            return sources.map(function (source) {
	                return '<option value=\'' + source.id + '\'\n                ' + (source.id === _this3.options.INITIAL_SOURCE ? 'selected=\"true\"' : '') + '>' + source.name + '</option>';
	            }).join();
	        }
	    }], [{
